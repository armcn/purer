...
test_that("pure_lte throws if with invalid value of .types", {

  int <- Int(1L)

  expect_error(
    pure_lte(.x = int, .y = int, .types = Chr("SS"))
  )

  expect_error(
    pure_lte(.x = int, .y = int, .types = Chr("s"))
  )

  expect_error(
    pure_lte(.x = int, .y = int, .types = Chr("se"))
  )

})

test_that("pure_lte returns correct values", {
