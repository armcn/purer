...
test_that("pure_gte throws if with invalid value of .types", {

  int <- Int(1L)

  expect_error(
    pure_gte(.x = int, .y = int, .types = Chr("SS"))
  )

  expect_error(
    pure_gte(.x = int, .y = int, .types = Chr("s"))
  )

  expect_error(
    pure_gte(.x = int, .y = int, .types = Chr("se"))
  )

})

test_that("pure_gte returns correct values", {
