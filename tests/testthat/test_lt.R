...
test_that("pure_lt throws if with invalid value of .types", {

  int <- Int(1L)

  expect_error(
    pure_lt(.x = int, .y = int, .types = Chr("SS"))
  )

  expect_error(
    pure_lt(.x = int, .y = int, .types = Chr("s"))
  )

  expect_error(
    pure_lt(.x = int, .y = int, .types = Chr("se"))
  )

})

test_that("pure_lt returns correct values", {
...
test_that("%<vv% returns correct values", {
  ...
  expect_equal(
    Int_(c(1L, 1L)) %<vv% Int_(c(1L, 2L)),
    Lgl_(c(FALSE, TRUE))
  )

  expect_equal(
    Dbl_(c(1, 1)) %<vv% Dbl_(c(1, 2)),
    Lgl_(c(FALSE, TRUE))
  )
...
test_that("%<vv% throws if an Int and Dbl vectors are compared", {
